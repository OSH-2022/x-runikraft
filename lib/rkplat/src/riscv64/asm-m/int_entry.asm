...
__rkplat_int_except_entry:
    csrrw t1, mscratch, t1 #读取 HartLocal
    sd t0,(t1)             #暂存t0 到 _reg_space
    csrr t0, mcause
    bgez t0,__rkplat_except_handle_entry    #mcause>=0，即最高位是0，说明是异常
    ld t0,(t1)
    csrrw t1, mscratch, t1  #还原t1、t0和mscratch
...
__rkplat_except_handle_entry:
    ...
    addi sp,sp, -152
    sd t0,(sp)
    csrrw t0,mscratch,t1 #还原mscratch，并且把t1原来的值加载到t0
    sd t0,8(sp)
    sd t2,16(sp)
    sd t3,24(sp)
    sd t4,32(sp)
    ...
    ld t1,136(sp)
    addi t0,t0,4
    csrw mepc,t0    #将mepc还原成引发异常的指令的下一条指令
    csrw mstatus,t1
    ld t0,(sp)
    ld t1,8(sp)
    ld t2,16(sp)
    ...
    ld a7,112(sp)
    ld ra,120(sp)
    ld sp,144(sp)   #不允许出现多重异常，所以mscratch里储存的hartsp不应该改变，所以无需还原
    mret
