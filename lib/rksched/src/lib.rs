...
impl<'a> RKsched<'a> {
    ...
    fn get_idle(&'a self) -> &'a mut RKthread<'a> {
        let idle: &'a mut RKthread<'a> = unsafe { &mut *(&self.idle as *const RKthread<'a> as *mut RKthread<'a>) };
        idle
    }

    fn thread_create(&mut self, name: *const char, attr: &'a mut RKthreadAttr, function: fn(*mut u8), arg: *mut u8) {
        // TODO
    }

    fn thread_destroy(&mut self, t: &'a mut RKthread<'a>) {
        // TODO
    }

    fn thread_kill(&mut self, t: &'a mut RKthread<'a>) {
        // TODO
    }

    fn thread_switch(&mut self, prev: &'a mut RKthread<'a>, next: &'a mut RKthread<'a>) {
        // TODO
    }
}